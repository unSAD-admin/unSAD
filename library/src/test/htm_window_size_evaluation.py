...
def window_size_experiment(path, window_size=50):
    files = {}
    output = []
    print ('hello')
    for dirpath, dirnames, filenames in os.walk(path):
        ...
        for filename in filenames:
            ...
            with open(filepath, newline='') as f:
                ...
                for idx, row in enumerate(reader):
                    #ts, value, label, anomaly, res_label
                    data.append(row)
            for window_size in range(10, 210, 10):
                tot_window = 0
                TP = 0
                TN = 0
                FP = 0
    ...
    writecsv(output, 'window_size_expriment.csv')

if __name__ == '__main__':
    window_size_experiment('2019-10-10_00-09-18_res')
